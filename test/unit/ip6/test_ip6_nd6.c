...
static u8_t host_mac[6] = {1, 2, 3, 4, 5, 7};
...
static err_t router_ip6_output(struct netif *netif, struct pbuf *p,
...
static err_t host_ip6_output(struct netif *netif, struct pbuf *p,
                             const ip6_addr_t *ipaddr) {
  LWIP_UNUSED_ARG(netif);
  LWIP_UNUSED_ARG(p);
  LWIP_UNUSED_ARG(ipaddr);
...
static struct pbuf *create_nd6_rio_test_packet(ip6_addr_t *dst_addr,
  ...
  route_lifetime = lwip_htonl(3600);
...
static err_t testif_init_host(struct netif *netif) {
  netif->name[0] = 'h';
  netif->name[1] = 's';
  netif->output_ip6 = host_ip6_output;
...
static err_t testif_init_router(struct netif *netif) {
  netif->name[0] = 'r';
  netif->name[1] = 't';
  netif->output_ip6 = router_ip6_output;
